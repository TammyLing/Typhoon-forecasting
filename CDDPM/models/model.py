...
class Palette(BaseModel):
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ret_path.append(f'{self.path[idx]}')
            ret_result.append({
                'gt': (self.gt_image[idx].detach().cpu().numpy()+1)/2,
                'cond': (self.cond_image[idx].detach().cpu().numpy()+1)/2,
                'out_vis': (self.visuals[idx-self.batch_size].detach().cpu().numpy()+1)/2,
                'out': (self.output[idx].detach().cpu().numpy()+1)/2,
            })
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            if self.iter % self.opt['train']['log_iter'] == 0:
                ...
                for key, value in self.get_current_visuals().items():
                    # Assuming value is a 4-channel tensor with shape [N, 4, 64, 64]
                    for i in range(value.size(0)):  # Loop through the batch
                        image = value[i]  # Get the ith image from the batch
                        channels = torch.split(image, 1, dim=0)  # Split the channels to get four [1, 64, 64] tensors
                        channel_names = ['u10', 'v10', 'sp', 't2m']
                        for j, (channel, name) in enumerate(zip(channels, channel_names)):
                            self.writer.add_image(f"{key}_{name}_{i}", channel.squeeze(0).unsqueeze(0), self.iter, dataformats='CHW')
    ...
    def val_step(self):
        ...
        saved = False
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.test_loader):
                ...
                for key, value in self.get_current_visuals(phase='val').items():
                    if not saved:
                        channels = torch.split(value, 1, dim=1)  # 按通道分离
                        for i, channel in enumerate(['u10', 'v10', 'sp', 't2m']):
                            self.writer.add_images(f"{key}_{channel}", channels[i])
                        saved = True
                for key, value in self.get_current_visuals(phase='val').items():
                    for i in range(value.size(0)):  # Loop through the batch
                            image = value[i]  # Get the ith image from the batch
                ...
                self.writer.save_np_arrays(self.save_current_results())
    ...
    def test(self):
        ...
        saved = False
        with torch.no_grad():
            for test_data in tqdm.tqdm(self.test_loader):
                ...
                for key, value in self.get_current_visuals(phase='test').items():
                    if not saved:  
                        channels = torch.split(value, 1, dim=1) 
                        for i, channel in enumerate(['u10', 'v10', 'sp', 't2m']):
                            self.writer.add_images(f"{key}_{channel}", channels[i])
                    saved = True
