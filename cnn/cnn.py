...
class ResidualBlock(nn.Module):
    """
    A Residual Block that consists of two convolutional layers with batch normalization and dropout.
    """
...
class ImprovedCNN(nn.Module):
    """
    An Improved CNN model that consists of four residual blocks and four transposed convolutional layers.
    """
...
def train_cnn(train_loader, val_loader, model, optimizer, device, num_epochs=100, patience=20):
    """
    ...
    for epoch in range(num_epochs):
        ...
        val_loss = evaluate_cnn(val_loader, model, device, dataset=None)
        ...
        for i, data in enumerate(loader):
            ...
            if i == 0:
                for j, target_name in enumerate(['U10', 'V10', 'Pressure', 'Temperature']):
                    ...
                    plt.tight_layout()
                    plt.savefig(f'cnn_prediction_vs_true_{target_name}_plot_best_params.png')
                    plt.show()
                    plt.close(fig)
            break
...
if __name__ == '__main__':
    ...
    # Finetune the CNN model
    print(f"Finetuning CNN Model:")
    batch_sizes = [32, 16, 8]
    ...
    print(f"Testing best CNN Model:")
    model.load_state_dict(torch.load('cnn_best_model.pth'))
    test_loss = evaluate_cnn(test_loader, model, device, dataset)
    print(f"Test Loss: {test_loss:.4f}")

    # Visualize the prediction of the best CNN model
